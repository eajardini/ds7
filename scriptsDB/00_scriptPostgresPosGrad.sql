...
um usuário e um schema para ele.
2) O usuário remoto deverá adicionar o SCHEMA com seu nome antes de CRIAR uma tabela assim:
   create table userds7.cliente (.....);
3) O usuário remoto deverá adicionar o SCHEMA com seu nome antes de PROJETAR uma tabela assim:
   select * from  userds7.cliente;
   
4) Somente use as linhas abaixo caso haja usuários remotos

-- CREATE SCHEMA userds7 AUTHORIZATION userds7;

*/

-- Apaga os bancos de dados para aula
DROP DATABASE IF EXISTS vdd_erp;
DROP DATABASE IF EXISTS vdd_dw;

-- Cria banco de dados do ERP
select
'
CREATE DATABASE vdd_erp
    WITH 
    OWNER = userds7      
    CONNECTION LIMIT = -1;
'
WHERE NOT EXISTS (SELECT FROM pg_database WHERE datname = 'vdd_erp')\gexec  
  
ALTER DATABASE vdd_erp SET datestyle TO 'SQL, DMY';
    
-- Cria banco de dados do DW
select
'
CREATE DATABASE vdd_dw
    WITH 
    OWNER = userds7     
    CONNECTION LIMIT = -1;
'
WHERE NOT EXISTS (SELECT FROM pg_database WHERE datname = 'vdd_dw')\gexec

ALTER DATABASE vdd_dw SET datestyle TO 'SQL, DMY';


-- Conecta com o Database vdd_erp
\c vdd_erp


-- Passo ERP1 - Criação das tabelas dentro do DB vdb_erp
...
FROM prazoEntrega_list, codCliente_list, codVendedor_list, totalPedido_list, 
     dataPedido_list, generate_series(1000,10000) ped
--limit 10
;
...
FROM numPedido_list, codProduto_list, qtade_list, valorVenda_list, 
     valorCusto_list, generate_series(1000,90000) ip
--limit 10
;
